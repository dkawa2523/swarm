...
def execute_apply_comsol(
    ...
    *,
    ...
    """Apply positive-column lookup tables to a fresh model copy."""
    ...
    return execute_comsol_java_source(
...
def prepare_positive_column_run(
    ...
    *,
    ...
    try:
        validate_bundle_selection(mapping.bundle.path)
    except ClosureSelectionError as exc:
        raise PositiveColumnWorkflowError(str(exc)) from exc
...
def execute_positive_column_run(
    ...
    *,
    ...
    run_summary = execute_generated_comsol_java(
        comsol_run_context(plan.mapping),
        plan.run_java_path,
...
def write_positive_column_run_summary(
    ...
    *,
    ...
    executed_voltages = _executed_voltage_sequence(run_summary)
...
def _executed_voltage_sequence(summary: Any | None) -> tuple[float, ...] | None:
    """Read positive-column continuation markers from a run log."""

    stdout_paths = getattr(summary, "stdout_paths", ())
    if not stdout_paths:
        return None
    path = Path(stdout_paths[-1])
    if not path.is_file():
        return None
