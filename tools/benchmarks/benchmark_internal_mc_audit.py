...
from tools.benchmarks.common import case_audit_metadata, metadata_value
...
def _tail_rows(result) -> list[dict[str, object]]:
    ...
    for case in result.cases:
        ...
        cumulative = 0.0
        for index, (energy, eedf, count) in enumerate(
            zip(case.energy_eV, case.eedf, case.eedf_counts, strict=False)
        ):
            count_int = int(count)
            effective_count = (
            ...
            width = (
                float(widths[index])
                if index < len(widths)
                and np.isfinite(widths[index])
                and widths[index] > 0.0
                else float("nan")
            )
            probability = (
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("outputs/benchmarks/internal_mc_audit")
    )
