...
def _energy_quantile(
    energy: np.ndarray, eedf: np.ndarray, widths: np.ndarray, q: float
) -> float:
...
def _major_rate_difference(
    reference: SwarmCaseResult, candidate: SwarmCaseResult
) -> float:
    ref = {
...
def eedf_metrics(
    ...
    return {
        ...
        "mean_energy_relative_difference": abs(
            candidate.mean_energy_eV - reference.mean_energy_eV
        )
...
def classify_eedf_failure(
    ...
    *,
    ...
    def add(
        category: str, evidence: str, code_area: str, fix: str, severity: str
    ) -> None:
        rows.append(
            {
                "category": category,
    ...
    elif solver_method == "pn_closure_direct" and eedf_l1 > 0.03:
        add(
            "expected_physics_difference",
