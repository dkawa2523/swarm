...
def load_benchmark_config(
    ...
    return load_config_from_raw(product_raw, config_path), reference_configs
...
def _matching_references(
    ...
    *,
    ...
    configs = [
        ...
        if desired == "all"
        or ref.id == desired
        or (desired == "mcig" and ref.id == "bolsig_plus")
    ]
    if bolsig_output is not None:
        configs = [ref for ref in configs if ref.id != "bolsig_plus"]
        configs.append(
            ExternalReferenceConfig(
                id="bolsig_plus",
                path=bolsig_output,
    ...
    for solver, method, lmax in solver_variants:
        for eover in cfg.run.e_over_n_Td:
            try:
                ...
                affected_solver = (
                    f"{solver}_lmax{lmax}"
                    if solver == "multi_term" and lmax is not None
                    else solver
                )
                failures.append(
                    _failure(
...
def _mc_confidence_status(
    ...
    for scalar, attr in (
        ...
        total += 1
        if abs(
            float(getattr(candidate, attr)) - reference_case.scalars[scalar]
        ) <= float(ci[scalar]):
            covered += 1
    if total == 0:
        return "unknown", ""
    ...
    return (
        "pass_within_mc_uncertainty" if covered == total else "outside_mc_uncertainty"
    ), coverage
...
def _classify_bolsig_mismatch(
    ...
    *,
    ...
    if (
        metrics.get("mean_energy_relative_difference", 0.0)
        > BOLSIG_THRESHOLDS["mean_energy_relative_difference"]
    ):
        rows.append(
            _failure(
    ...
    if (
        ...
        > BOLSIG_THRESHOLDS["major_rate_relative_difference"]
    ):
        rows.append(
            _failure(
    ...
    *,
    ...
    if metrics.get("eedf_relative_l1", 0.0) > MCIG_THRESHOLDS["eedf_relative_l1"]:
        category = (
            ...
            if candidate_solver == "multi_term"
            and str(candidate_method) == "pn_closure_direct"
    ...
    if (
        metrics.get("mean_energy_relative_difference", 0.0)
        > MCIG_THRESHOLDS["mean_energy_relative_difference"]
        and candidate_solver == "two_term"
    ):
        rows.append(
            _failure(
...
def _write_plot(
    ...
    try:
        ...
        raise RuntimeError(
            "--plot requires matplotlib; install the plot extra"
        ) from exc
    ...
    fig, axes = plt.subplots(
        rows, cols, figsize=(6.0 * cols, 4.5 * rows), squeeze=False
    )
...
def run_benchmark(
    ...
    *,
    ...
    for reference_case in reference_cases:
        if (
            reference_case.reference_id
            not in {_reference_key(reference), "bolsig_plus"}
            and _reference_key(reference) != "all"
        ):
            continue
        for candidate in [
            case
            ...
            thresholds = (
                MCIG_THRESHOLDS
                if reference_case.reference_id == "mcig"
                else BOLSIG_THRESHOLDS
            )
            for threshold_name, threshold in thresholds.items():
                if metrics.get(threshold_name, 0.0) > threshold:
                    status = "FAIL"
            if (
                reference_case.reference_id == "mcig"
                and confidence_status == "pass_within_mc_uncertainty"
            ):
                status = "PASS_WITHIN_MC_UNCERTAINTY"
            if (
                reference_case.reference_id == "mcig"
                and angular_model_status != "match"
            ):
                status = "DEGRADED"
            common = {
            ...
            summary_rows.append(
                {
                    **common,
                    ...
                    "ionization_rate_relative_difference": _rate_metric(
                        metrics, "ionization"
                    ),
                    "excitation_rate_relative_difference": _rate_metric(
                        metrics, "excitation"
                    ),
    ...
    if _reference_key(reference) in {"all", "mcig"}:
        ...
        bolsig_cases = [
            case for case in reference_cases if case.reference_id == "bolsig_plus"
        ]
        for mcig_case in mcig_cases:
            for bolsig_case in bolsig_cases:
                if abs(mcig_case.e_over_n_Td - bolsig_case.e_over_n_Td) > 1.0e-9:
                    continue
                from tools.benchmarks.references.common import reference_to_swarm_case
                ...
                common = {
                    ...
                    if mcig_case.metadata.get("uncertainty_unavailable", False)
                    else "reported",
                    "angular_model_status": angular_model_status,
                    "mc_confidence_interval_coverage": "",
                }
    ...
    written_plot = (
        _write_plot(out_dir, base_name, reference_cases, solver_cases) if plot else None
    )
    status = "PASS"
    ...
    selected_thresholds = (
        MCIG_THRESHOLDS if _reference_key(reference) == "mcig" else BOLSIG_THRESHOLDS
    )
    report_path.write_text(
        "\n".join(
            [
                ...
                "Multi-term is evaluated as an independent PN model; it is not "
                "forced to reproduce the two-term solution.",
...
def main() -> None:
    ...
    parser.add_argument(
        "--reference",
        default="bolsig",
        choices=["bolsig", "bolsig_plus", "mcig", "all"],
    )
    ...
    parser.add_argument(
        "--angular-model",
        choices=["isotropic", "mcig_default", "unknown"],
        default=None,
    )
