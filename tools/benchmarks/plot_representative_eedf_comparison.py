...
def comsol_f0_eV_m32(
    ...
    return power_g * mean_energy_eV**-1.5 * beta_1 * np.exp(-(scaled**power_g))
...
def _plot_overview(
    ...
    fig.subplots_adjust(
        left=0.09, right=0.98, bottom=0.07, top=0.88, hspace=0.28, wspace=0.16
    )
        ...
        summary_rows.append(
            {
                ...
                "solver_mean_energy_relative_difference": abs(
                    mc.reported_mean_energy_eV - two_term.reported_mean_energy_eV
                )
                / two_term.reported_mean_energy_eV,
...
def _parser(repo_root: Path) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=base
        / "monte_carlo_function_eedf_restricted_lmea_bundle"
        / "mixture_0000",
    )
    parser.add_argument(
        "--two-term-bundle",
