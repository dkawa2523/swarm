...
from tools.benchmarks.common import write_csv
...
def _metric_row(
    ...
    *,
    ...
    return {
        "E_over_N_Td": eover,
        "reference": reference,
        "candidate": _solver_label(candidate)
        if not isinstance(candidate, str)
        else candidate,
        "candidate_method": ""
        if isinstance(candidate, str)
        else candidate.metadata.get("solver_method", ""),
        "candidate_lmax": ""
        if isinstance(candidate, str)
        else candidate.metadata.get("lmax", ""),
...
def _find_reference(
    cases: list[ReferenceCaseResult], eover: float, ref_id: str
) -> ReferenceCaseResult | None:
    for case in cases:
    ...
    *,
    ...
    for eover in cfg.run.e_over_n_Td:
        ...
        if two is not None and direct1 is not None:
            ...
            statuses["multi_term_lmax1_vs_two_term"] = "PASS"
            if metrics.get("eedf_relative_l1", 0.0) > 0.03:
                statuses["multi_term_lmax1_vs_two_term"] = "DIFFERENT"
                physics_limit = True
                likely.append("independent_solver_model_difference")
                _add_failure(
                    failure_rows,
                    "independent_solver_model_difference",
                    f"eedf_relative_l1={metrics['eedf_relative_l1']:.3e}",
                    eover,
                    "two_term->multi_term_lmax1",
                    "independent two-term and PN operators",
                    "inspect PN lmax convergence and angular-model assumptions",
                    "medium",
                )
        else:
            statuses["multi_term_lmax1_vs_two_term"] = "missing"
            if two is not None:
                implementation_bug = True
                likely.append("multi_term_lmax1_execution_failure")
                _add_failure(
                    failure_rows,
                    "multi_term_lmax1_execution_failure",
                    "multi_term pn_closure_direct lmax=1 result is missing",
                    eover,
                    "two_term->multi_term_lmax1",
                    "multi_term direct lmax1 execution",
                    "inspect solver_execution_failure rows and PN convergence",
                    "critical",
                )
        ...
        if bolsig is not None and two is not None:
            ...
            statuses["two_term_vs_bolsig"] = (
                "PASS" if not _is_bad(metrics, BOLSIG_THRESHOLDS) else "FAIL"
            )
        ...
        if bolsig is not None and mcig is not None:
            ...
            eedf_rows.append(
                _metric_row(eover, "mcig", "reference:bolsig_plus", metrics)
            )
            statuses["mcig_vs_bolsig"] = (
                "PASS" if not _is_bad(metrics, MCIG_THRESHOLDS) else "DEGRADED"
            )
        ...
        if mcig is not None:
            for candidate in [
                case for case in solver_cases if abs(case.e_over_n_Td - eover) < 1.0e-9
            ]:
                ...
                if _is_bad(metrics, MCIG_THRESHOLDS):
                    if candidate.solver == "multi_term" and str(
                        candidate.metadata.get("lmax", "")
                    ) not in {"", "1"}:
                ...
                if (
                    metrics.get("major_rate_relative_difference", 0.0)
                    > MCIG_THRESHOLDS["major_rate_relative_difference"]
                    and metrics.get("eedf_relative_l1", 0.0)
                    <= MCIG_THRESHOLDS["eedf_relative_l1"]
                ):
            ...
            statuses["multi_lmax_gt1_vs_mcig"] = (
                ...
                if high_l_rows
                and all(
                    float(row.get("eedf_relative_l1", 0.0))
                    <= MCIG_THRESHOLDS["eedf_relative_l1"]
                    for row in high_l_rows
                )
                else "DEGRADED"
                if high_l_rows
                else "missing"
        ...
        if not likely:
            likely.append(
                "no_external_reference_available"
                if bolsig is None and mcig is None
                else "pass"
            )
        overall = (
            "FAIL"
            ...
            if physics_limit
            or mc_uncertain
            or any("mismatch" in item for item in likely)
            else "PASS"
        )
        matrix_rows.append(
            {
                ...
                "multi_term_lmax1_vs_two_term": statuses.get(
                    "multi_term_lmax1_vs_two_term", "missing"
                ),
                "mcig_vs_bolsig": statuses.get("mcig_vs_bolsig", "missing"),
                "multi_lmax_gt1_vs_mcig": statuses.get(
                    "multi_lmax_gt1_vs_mcig", "missing"
                ),
    ...
    if any(row["overall_status"] == "FAIL" for row in matrix_rows):
        ...
        status = "DEGRADED"
    if all(
        row["likely_cause"] == "no_external_reference_available" for row in matrix_rows
    ):
        status = "SKIP_EXTERNAL_REFERENCE"
    written_plot = (
        _write_plot(out_dir, "ar_triage", references, solver_cases) if plot else None
    )
    paths["report"].write_text(
        "\n".join(
            [
                ...
                else "Plot: not requested or no reference cases",
                "",
    ...
    if fail_on_code_regression and any(
        row["implementation_bug"] for row in matrix_rows
    ):
        raise RuntimeError("triage detected code regression")
    if fail_on_physics_mismatch and any(
        row["physics_model_limitation"] for row in matrix_rows
    ):
...
def _recommend(likely: list[str]) -> str:
    causes = set(likely)
    if "independent_solver_model_difference" in causes:
        return "inspect PN lmax convergence and angular-model assumptions"
    if "multi_term_lmax1_execution_failure" in causes:
        return "inspect the multi-term solver execution failure"
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/benchmarks/ar_bolsig_mcig_triage.yaml"),
    )
    parser.add_argument("--bolsig-output", type=Path, default=None)
    parser.add_argument("--mcig-output", type=Path, default=None)
    parser.add_argument(
        "--run-bolsig", default=None, help="external BOLSIG+ command template"
    )
    parser.add_argument(
        "--run-mcig", default=None, help="external MCIG command template"
    )
