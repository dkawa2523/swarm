...
from tools.benchmarks.eedf_compare import compare_eedf_cases, normalize_eedf
...
def _normalized_distribution(
    ...
    *,
    ...
    return Distribution(
        ...
        else np.asarray(sample_count, dtype=float),
        effective_sample_count=(
            None
...
def _distributions_from_internal_tail(
    ...
    *,
    ...
    for (case_id, eover), group in frame.groupby(
        ["case_id", "E_over_N_Td"], sort=False
    ):
        ...
        distributions.append(
            _normalized_distribution(
                ...
                metadata=metadata,
                sample_count=(
                    pd.to_numeric(ordered["sample_count"], errors="coerce").to_numpy(
                        float
                    )
...
def _band_integrals(
    ...
    mean_contribution = float(
        np.sum(
            0.5
            * dist.eedf_eV_inv
            * (overlap_right**2 - overlap_left**2)
            * (overlap > 0.0)
        )
    )
...
def _band_rows(distributions: list[Distribution]) -> list[dict[str, object]]:
    ...
    for dist in distributions:
        for left, right in DEFAULT_BANDS:
            ...
            if dist.effective_sample_count is not None and len(
                dist.effective_sample_count
            ) == len(mask):
...
def _assessment_rows(
    ...
    by_key = {(dist.source, float(dist.e_over_n_Td)): dist for dist in distributions}
    rows: list[dict[str, object]] = []
    for row in comparisons:
        ...
        xs_fraction = _float_metadata(
            metadata, "mc_energy_samples_above_xs_max_fraction"
        )
        tail_status = str(metadata_value(metadata, "mc_tail_uncertainty_status"))
        tail_probability = (
            _tail_probability(candidate, 40.0) if candidate is not None else 0.0
        )
    ...
    *,
    ...
    for row in metrics:
        lines.append(
            ...
            "E90={E90_eV:.6g} eV, tail>40={tail_survival_gt_40_eV:.6g}".format(**row)
        )
    if comparisons:
